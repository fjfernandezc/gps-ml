...
extern int testeclipse_(gtime_t time, int sat, const nav_t *nav, const double *rs)
{
    ...
    if ((r=norm(rs,3))<=0.0) return 0;
    
    /* only block IIA */
    if (*type&&!strstr(type,"BLOCK IIA")) return 0;
    ...
    /* test eclipse */
    if (ang<PI/2.0||r*sin(ang)>RE_WGS84) return 0;
