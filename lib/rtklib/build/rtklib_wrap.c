...
pcvs_t *pcvnew(void)
{
    ...
    if (!(pcv = (pcvs_t *)calloc(1, sizeof(pcvs_t)))) {
        return NULL;
    }
    return pcv;
}
...
void preceph(char *file, nav_t *nav)
{
    nav->ne=nav->nemax=0;
...
void precclk(char *file, nav_t *nav)
{   
    nav->nc=nav->ncmax=0;
...
void setsatpcv(gtime_t time, nav_t *nav, const pcvs_t *pcvs)
{
    pcv_t *pcv,pcv0={0};
    int i;
